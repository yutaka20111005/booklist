...
#タイトル
st.title("Machine Lerning application (機械学習アプリ)")
st.write("by streamlit")

# 以下をサイドバーに表示
st.sidebar.markdown("### csvファイルを選択して下さい")
#ファイルアップロード
...
if uploaded_files:
    ...
    #機械学習のタイプにより以下の処理が分岐
    if ml_menu == "重回帰分析":
            st.markdown("#### 重回帰分析、始め！")
            execute = st.button("ここをクリック")
    ...
    #ロジスティック回帰分析を選択した場合
            ...
            st.markdown("#### ロジスティック回帰分析、始め！")
            execute = st.button("ここクリック")
